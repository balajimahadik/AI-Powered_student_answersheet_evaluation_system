...
def modalAnswerLoad(QuesArr,BlockArr,DigArr,pdf_path):
    BlockDiagram = {}
    DiagramImage = {}
    DiagramMap = {}
    ...
    for page_num in range(len(pdf_document)):
        ...
        image_path = f"./static/ImageData/Modal Answer/Pages/page_{page_num+1}.png"
        ...
        if BlockArr[page_num]==1:
            ...
            count,start = extractblkdig(image_path,page_num+1,"Modal Answer")
            for i in range(start,start+count):
                cropped_image_path = f"./static/ImageData/Modal Answer/cropped_image_{i}.png"
                block_map = blockDigEval(cropped_image_path)
                BlockDiagram.__setitem__(page_num+1,block_map)
        elif DigArr[page_num]== 1:
            count,start = extractblkdig(image_path,page_num,"Modal Answer")
            for i in range(start,start+count):
                cropped_image_path = f"./static/ImageData/Modal Answer/cropped_image_{i}.png"
        ...
        response = model.generate_content(
            [sample_file, "Extract text only from given image"])
